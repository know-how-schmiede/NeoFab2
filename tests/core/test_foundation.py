...
def test_explicit_migration_is_repeatable_and_preserves_data(app):
    ...
    with sqlite3.connect(path) as db:
        ...
        assert {r[0] for r in db.execute("SELECT name FROM sqlite_master WHERE type='table'")} == {
            "core_settings", "alembic_version", "core_users", "core_sessions", "core_login_attempts", "core_user_options", "core_files", "core_mail_outbox", "core_account_tokens", "core_account_limits", "core_audit_events", "core_worker_status", "core_user_imports"}
    client = app.test_client()
